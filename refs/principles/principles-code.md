...
Loaded **on top of** the global base (`refs/principles/principles-global.md`) when `refs/detect.md` resolves `prompt_type = code`. The `type` column marks `additive` / `subtractive` for ranking against the global pool.
